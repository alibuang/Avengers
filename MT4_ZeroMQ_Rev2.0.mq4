...
void InterpretZmqMessage(Socket &pSocket, string& compArray[]) {
   ...
   if(compArray[0] == "TRADE" && compArray[1] == "OPEN"){
    ...
    else if(compArray[0] == "MAGICNUM")
      switch_action = 13;
      
    else if(compArray[0] == "PROFIT_MAGIC"){
      magic_number = compArray[1];
      switch_action = 14;      
    }
    else if(compArray[0] == "CLOSE_MAGICNUM"){
      magic_number = compArray[1];
      switch_action = 15;      
    }
   ...
   switch(switch_action) 
   {
      ...
      case 14:
         ...
         ret = "PROFIT_MAGIC"; 
         ret = ret + "|" + Get_ProfitMagicNum(magic_number);
         //Print(ret);
      ...
      case 15:
         ...
         ret = "CLOSE_MAGIC"; 
         err_cd = Close_ByMagic(magic_number);
...
//|   This function is to Count number of trades                                                               |
//+------------------------------------------------------------------+
string Get_ProfitMagicNum(int magic_number)
{
            double profit;
            int trade;
            for(trade=OrdersTotal()-1;trade>=0;trade--)
...
//+------------------------------------------------------------------+
//|   This function is to get all magic_num                                                               |
//+------------------------------------------------------------------+
string Get_MagicNum()
{
            int count=0;
            ...
            for(trade=OrdersTotal()-1;trade>=0;trade--)
              {
               int order_chk = OrderSelect(trade,SELECT_BY_POS,MODE_TRADES);
               if(OrderMagicNumber()!= 0 && OrderMagicNumber()!= mag_num){               
                  
                  mag_num = OrderMagicNumber();
                  magnum_Str = magnum_Str + "|" + IntegerToString(mag_num);
...
// This function is to close all open position
//---------------------------------------------------------
bool Close_ByMagic(int magic_number){
