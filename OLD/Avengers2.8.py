...
from ffcalnews2 import chk_news
...
def get_abs_strength(higher_tf, lower_tf, higher_shift, lower_shift):
        
    abs_d1, dt_d1 = Get_Strength_Index(higher_tf, higher_shift)
    abs_h1, dt_h1 = Get_Strength_Index(lower_tf, lower_shift)
...
def Trade_News():
    ...
    #check for news time
    news_df = chk_news(impact=impact, window = window)
#    print(news_df)
    
    news_inspected = None
    
    # Open trade 5 minutes prior to News
    for n in range(len(news_df)):
        
        temp_datetime = datetime.strptime(news_df.loc[n].datetime, '%Y-%m-%d %H:%M:%S')
        news_datetime = cal_tz.localize(temp_datetime).astimezone(local_tz) 
        trade_datetime = news_datetime - timedelta(minutes= 5)

        if datetime.now().hour == trade_datetime.hour and \
        datetime.now().minute == trade_datetime.minute:
            
            #if currency already check then skip
            if news_df.loc[n].currency == news_inspected:
                continue  
            else:
                news_inspected = news_df.loc[n].currency
            
            print('YES it is a trade time')
            
            #get signal to buy or to sell
            strength_index, date_time = Get_Strength_Index(timeframe.M15, 1)         
            
            
            for s in range(len(strength_index)):
                if strength_index.loc[s].Currency == news_df.loc[n].currency:
                    effective_si = strength_index.loc[s].ABS_strength
                    break
            
            trade_symbol = 'AUDUSD.lmx'
            broker1.get_price([trade_symbol]) #input symbol in form of list
            
            if effective_si > 0:
                order_type = sig.buy
                price = broker1.ask[0] #bid in form of list
                
                
            elif effective_si < 0:
                order_type = sig.sell
                price = broker1.bid[0] #ask in form of list
            ...
            broker1.send_order(order_type, trade_symbol, price, lot, slip, sl, tp, comments)
            
        print ('Trade time ',trade_datetime.strftime('%Y-%m-%d %I:%M:%S %p'))
...
def Send_Notification():
    ...
    #publish status every 15 minutes and there is news
    if len(news_df) > 0 and (datetime.now().minute % 15 == 1 or first_run ):
                   
        #check candle status
        higher_candle_status = check_candle(Symbols, timeframe.H1, 1)  
        lower_candle_status = check_candle(Symbols, timeframe.M15, 1)  
        print('H1 status:', higher_candle_status, '\nM15 status:', lower_candle_status)
        
        #if candle status is good then check strength index and send telegram
        if higher_candle_status and lower_candle_status:
            ...
            #get strength index
            higher_si, lower_si, higher_dt, lower_dt = get_abs_strength(timeframe.H1, timeframe.M15, 1, 1)
            
            #convert the time to the correct timezone
            higher_dt = mt4_tz.localize(higher_dt).astimezone(local_tz)
            lower_dt = mt4_tz.localize(lower_dt).astimezone(local_tz)
            h = higher_dt.strftime('%Y-%m-%d %I:%M:%S %p')
            l = lower_dt.strftime('%Y-%m-%d %I:%M:%S %p')
            print('higher= ',h , higher_dt.tzinfo)
            print('lower=', l , lower_dt.tzinfo )
            
            #re-check candle if not updated                
            prev_hour = datetime.now(local_tz) - timedelta(hours=1)
            if higher_dt.hour != prev_hour.hour:
                print('H1 candle not updated, rechecking ...')
                sleep(5)
                return
                        
            #plot chart
            plot_Chart(higher_si, lower_si, h, l)
            
            # send strength index chart to telegram
            timestamp = str(datetime.now().replace(microsecond=0))
            bot.send_photo(chat_id=chat_id, photo=open('index.png','rb'), \
            caption='absolute strength index\n'+timestamp, timeout=50)
            print('Send strength index chart to telegram.')
        ...
        for i in range(len(news_df)):
            ...
            temp_datetime = datetime.strptime(news_df.loc[i].datetime, '%Y-%m-%d %H:%M:%S')
            ...
            print(news_datetime.astimezone(local_tz).strftime('%Y-%m-%d %I:%M:%S %p'), \
                  news_df.loc[i].currency, news_df.loc[i].impact, news_df.loc[i].event, \
                  news_df.loc[i].actual, news_df.loc[i].forecast, news_df.loc[i].previous)
            
            #send news to telegram
            text_msg = '\n'.join(['News Time: '+news_datetime.astimezone(local_tz).strftime('%Y-%m-%d %I:%M:%S %p'), \
                  'Currency: '+ news_df.loc[i].currency, 'Impact: '+ news_df.loc[i].impact, 'Event: '+ news_df.loc[i].event, \
                  'Actual: '+ news_df.loc[i].actual, 'Forecast: '+ news_df.loc[i].forecast, 'Previous: '+ news_df.loc[i].previous])            
...
while True:    
    try:
        ...
        timenow = datetime.now().astimezone(local_tz).strftime('%Y-%m-%d %I:%M:%S %p')
        print(timenow, ': Running ... OK')
        first_run = False
