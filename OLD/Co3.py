...
symb = ['EURUSD.lmx','GBPUSD.lmx','AUDUSD.lmx', 'USDJPY.lmx', 'NZDUSD.lmx','USDCAD.lmx','USDCHF.lmx'\
        ,'GBPJPY.lmx','EURJPY.lmx','AUDJPY.lmx']
...
filtered_df =[]
for price in price_df:
    filtered_df.append(price.loc[(price.dt_MY.dt.time > start.time()) & (price.dt_MY.dt.time < end.time())])
...
X1 = pd.Series( filtered_df[pair_index_1].close)
X2 = pd.Series( filtered_df[pair_index_2].close)
tms = pd.Series( filtered_df[pair_index_1].dt_MY)
